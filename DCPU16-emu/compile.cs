    ...
    static class compile
    {
        ...
        public static ushort[] encode(string instruction,bool allowLiteralOptimization=true)
        {
            ...
            else
            {
                ...
                string partial="";
                bool instring=false;
                foreach (char c in instruction.Substring(spos).Trim())
                {
                    if (c!=',')
                        partial+=c;
                    if (c=='"')
                        ...
                        {
                            instring = false;
                            foreach (char clit in partial.ToCharArray())
                                output.Add((ushort)clit);
                            partial = "";
                            continue;
                        } else 
                            instring = true;
                    if (c == ',')
                    {
                        ...
                        else
                        {
                            ...
                            if (partial.StartsWith("0x"))
                                output.Add(ushort.Parse(partial.Substring(2), NumberStyles.HexNumber));
                            else if (partial.StartsWith("0b"))
                                output.Add(Convert.ToUInt16(partial, 2));
                            else output.Add(ushort.Parse(partial));
                        }
                    }
                }
                return output.ToArray();
